...
import { ADD_BLOGS, EDIT_BLOGS, GET_BLOGS } from "../../helpers/api_url_helper";
...
class BlogEdit extends React.Component {
    ...
    loadEdit = () => {
        ...
        get(GET_BLOGS, { params: params }).then(res => {
            if (res.status) {
                ...
                toastr.success("Blogs not found.");
    ...
    handleFormSubmit = (e) => {
        ...
        if (this.validator.allValid()) {
            ...
            axios({
                method: "post", url: `${process.env.REACT_APP_APIURL}${EDIT_BLOGS}`, data: formData, headers: {
                ...
                if (response.status) {
                    ...
                    toastr.success("Blogs update successful.");
                    // eslint-disable-next-line react/prop-types
                    const { history } = this.props;
                    // eslint-disable-next-line react/prop-types
                    history.push("/blogs");
            ...
            <React.Fragment>
                <div className="page-content">
                    ...
                    <Container fluid={true}>
                        <Breadcrumbs title="Blogs" path="/blogs" breadcrumbItem="Edit" />
                        <Row>
                            <Col xl="12">
                                <Card>
                                    <CardBody>
                                        <form onSubmit={this.handleFormSubmit}>
                                            <Row>
                                                <Col md="12" className="mb-3">
                                                    ...
                                                    <input name="title" placeholder="Type Blogs Title" defaultValue={this.state.title} type="text" className={"form-control"} onChange={this.handleInput} />
                                                    {this.validator.message("title", this.state.title, "required|string")}
                                                </Col>
                                                <Col md="3" className="mb-3">
                                                    <Label htmlFor="author">Blogs Author*</Label>
                                                    <input name="author" placeholder="Type Author" defaultValue={this.state.author} type="text" className={"form-control"} onChange={this.handleInput} />
                                                    {this.validator.message("author", this.state.author, "required|string")}
                                                </Col>
                                                <Col md="3" className="mb-3">
                                                    <Label htmlFor="blog_date">Blogs Date*</Label>
                                                    <input name="blog_date" placeholder="Type Blogs Date" defaultValue={this.state.blog_date} type="date" className={"form-control"} onChange={this.handleInput} />
