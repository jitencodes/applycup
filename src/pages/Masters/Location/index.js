...
import {GET_LOCATION} from "../../../helpers/url_helper";
...
const Location = () => {
  ...
  const TableColum = [
    ...
    {name: "Location", field: "name", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_LOCATION, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  const ImportCSV = (e) => {
    ...
    if (file !== undefined) {
      ...
      formData.append('master_name', 'Location-Master')
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Location | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" breadcrumbItem="Location/City" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td><Link to={`/location/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
