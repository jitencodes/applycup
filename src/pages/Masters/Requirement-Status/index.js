...
import {GET_REQUIREMENT_STATUS, EXPORT_MASTER} from "../../../helpers/api_url_helper";
...
const RequirementStatus = () => {
  ...
  const TableColum = [
    ...
    {name: "Requirement Status", field: "name", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_REQUIREMENT_STATUS, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  const ImportCSV = (e) => {
    ...
    if (file !== undefined) {
      ...
      formData.append('master_name', 'Requirement-Status')
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Requirement Status | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" breadcrumbItem="Requirement Status" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td><Link to={`/requirement-status/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
