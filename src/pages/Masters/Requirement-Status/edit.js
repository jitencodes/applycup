...
import {EDIT_REQUIREMENT_STATUS, GET_REQUIREMENT_STATUS} from "../../../helpers/api_url_helper";
import toastr from "toastr";
import PropTypes from "prop-types";


class RequirementStatusEdit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      requirement_title: "",
  ...
  componentDidMount() {
    this.getRequirementStatus(this.props.match.params.id)
  }

  getRequirementStatus(id) {
    get(GET_REQUIREMENT_STATUS, {params: {id: id}}).then(res => {
      if (res.data) {
        this.setState({
          id: res.data.id,
          requirement_title: res.data.name,
  ...
  handleFormSubmit = (e) => {
    ...
    if (this.validator.allValid()) {
      ...
      put(EDIT_REQUIREMENT_STATUS, formData).then(response => {
        if (response.status) {
      ...
      <React.Fragment>
        <div className="page-content">
          ...
          <Container fluid={true}>
            <Breadcrumbs title="Requirement Status" path="/requirement-status" breadcrumbItem="Edit" />
            <Row>
              <Col xl="12">
                <Card>
                  <CardBody>
...
RequirementStatusEdit.propTypes = {
...
export default RequirementStatusEdit;
