...
import {EXPORT_MASTER, GET_QUALIFICATION} from "../../../helpers/api_url_helper";
...
const Qualifications = () => {
  ...
  const TableColum = [
    ...
    {name: "Qualifications", field: "name", sortable: false},
    {name: "Ordering", field: "order_no", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_QUALIFICATION, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  const ImportCSV = (e) => {
    ...
    if (file !== undefined) {
      ...
      formData.append('master_name', 'Qualification')
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Manage Qualifications | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" path="/" breadcrumbItem="Qualifications" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td><Link to={`/qualifications/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
