...
import {GET_CLIENT_STATUS, EXPORT_MASTER} from "../../../helpers/api_url_helper";
...
const ClientStatus = () => {
  ...
  const TableColum = [
    ...
    {name: "Client Status", field: "name", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_CLIENT_STATUS, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  const ImportCSV = (e) => {
    ...
    if (file !== undefined) {
      ...
      formData.append('master_name', 'Client-Status')
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Client Status | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" breadcrumbItem="Client Status" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td><Link to={`/client-status/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
