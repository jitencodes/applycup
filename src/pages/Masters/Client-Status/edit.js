...
import {GET_CLIENT_STATUS, EDIT_CLIENT_STATUS} from "../../../helpers/api_url_helper";
import toastr from "toastr";
import PropTypes from "prop-types";


class ClientStatusEdit extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      status_title: "",
  ...
  componentDidMount() {
    this.getClientStatus(this.props.match.params.id)
  }

  getClientStatus(id) {
    get(GET_CLIENT_STATUS, {params: {id: id}}).then(res => {
      if (res.data) {
        this.setState({
          id: res.data.id,
  ...
  handleFormSubmit = (e) => {
    ...
    if (this.validator.allValid()) {
      ...
      put(EDIT_CLIENT_STATUS,formData).then(response => {
        if (response.status) {
...
export default ClientStatusEdit;
