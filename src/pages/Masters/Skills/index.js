...
import {GET_SKILLS, EXPORT_MASTER} from "../../../helpers/api_url_helper";
...
const Skills = () => {
  ...
  const TableColum = [
    ...
    {name: "Skill", field: "name", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_SKILLS, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  const ImportCSV = (e) => {
    ...
    if (file !== undefined) {
      ...
      formData.append('master_name', 'Skills')
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Skills | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" path="/" breadcrumbItem="Skills" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td>{parseInt(value.status) === 1 ? <span className="badge badge-pill bg-success font-size-13">Active</span> : <span className="badge badge-pill bg-danger font-size-13">Inactive</span>}</td>
                          <td><Link to={`/skills/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
