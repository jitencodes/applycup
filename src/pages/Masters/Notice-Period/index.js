...
import {GET_NOTICE_PERIOD} from "../../../helpers/api_url_helper";
...
const NoticePeriod = () => {
  ...
  const TableColum = [
    ...
    {name: "Notice Period", field: "name", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_NOTICE_PERIOD, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Manage Notice Period | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" path="/" breadcrumbItem="Notice Period" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td><Link to={`/notice-period/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
