...
import {GET_CLIENT_SOURCE, EXPORT_MASTER} from "../../../helpers/api_url_helper";
...
const ClientSource = () => {
  ...
  const TableColum = [
    ...
    {name: "Client Source", field: "name", sortable: false},
  ...
  useEffect(() => {
    ...
    get(GET_CLIENT_SOURCE, {params: params}).then(res => {
      hideLoader()
      if (res) {
  ...
  const ImportCSV = (e) => {
    ...
    if (file !== undefined) {
      ...
      formData.append('master_name', 'Client-Source')
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <MetaTags>
          <title>Client Source | {process.env.REACT_APP_PROJECTNAME}</title>
        </MetaTags>
        <div className="container-fluid">
          <Breadcrumbs title="Dashboard" path="/" breadcrumbItem="Client Source" />
          <Row>
                    ...
                    <table className="table table-striped">
                      ...
                      {!loader && tableData.map((value,index) => (
                        <tr key={++index}>
                          ...
                          <td><Link to={`/client-source/edit/${value.id}`}><i className="bx bxs-edit-alt"></i> </Link></td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
